			...
			<scroll-view class="keyword-box" v-show="!isShowKeywordList" scroll-y>
				...
				<view class="keyword-block">
					...
					<view class="keyword" v-if="forbid==''">
						<view v-for="(keyword,index) in hotKeywordList" @tap="doSearch(keyword._id)" :key="index">
							{{keyword._id}}
						</view>
					</view>
		...
		methods: {
			...
			inputChange(event) {
				...
				vk.callFunction({
					...
					if (res.code == 0) {
						this.keywordList = [];
						this.keywordList = this.drawCorrelativeKeyword(res.rows, keyword);
					}
				});
			},
			//高亮关键字
			...
			doSearch(keyword) {
				...
				uni.navigateTo({
					url: `/subPackages/archivesSelect/archivesSelect?tag=${keyword}`
				})
			},
			//保存关键字到历史记录
			saveKeyword(keyword) {
