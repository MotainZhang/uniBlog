					...
					<scroll-view class="panel-scroll-box" :scroll-y="enableScroll" @scrolltolower="loadMore">
						<view v-for="(item, index) in newsList" :key="index" class="news-item" @click="navToDetails(item)">
							...
							<view v-if="item.images.length > 0" :class="['img-list', 'img-list' + item.type, item.images.length === 1 && item.type === 3 ? 'img-list-single' : '']">
								...
								>
									<image class="img" :src="imgItem.imgsrc"></image>
...
export default {
	...
	methods: {
		...
		loadNewsList(type) {
			...
			// #endif
			if (type === 'refresh') {
				this.newsList = this.newsList.reverse(); //刷新前清空数组
			}
			//下拉刷新 关闭刷新动画
			if (type === 'refresh') {
			...
			if (type === 'add') {
				this.page++;
				this.getNewsList(tabItem);
			}
		},
		// 获取新闻接口
		getNewsList(tabItem) {
			tabItem.loadMoreStatus = 1;
			this.$u.api.getNews({ type: tabItem.name, url: `${this.apiBaseUrl}${tabItem.url}${this.page}-${this.pageSize}.html` }).then(res => {
				if (res.code == 200) {
					let jsonData = res.data[tabItem.alias];
					jsonData.forEach((item, index) => {
						if (item.imgextra) {
							item.images = item.imgextra;
							item.type = 3;
						} else {
							item.images = [];
							item.images.push({ imgsrc: item.imgsrc });
							item.type = 1;
						}
					});
					if (this.newsList.length == 0) {
						this.newsList = jsonData;
					} else {
						this.newsList.push(...jsonData);
					}
					tabItem.loadMoreStatus = 0;
				}
			});
